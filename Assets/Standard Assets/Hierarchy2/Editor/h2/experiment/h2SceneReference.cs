...
public class h2SceneReference {
	...
	static void ScanGameObject(GameObject go, bool force = false){
		...
		var compList = go.GetComponents<UnityEngine.Component>();
	...
	static void ScanComponent(UnityEngine.Component c, bool force = false){
		if (c == null) {
			nullCount++;
			return;
		}
		...
		for (var k = 0; k < props.Length; k++){
			...
			if (props[k].propertyType == SerializedPropertyType.ObjectReference) {
				...
				if (refObj is UnityEngine.Component){
					ScanComponent((UnityEngine.Component)refObj);
	...
	static public void LogReferences(GameObject go){
		var cList = go.GetComponents<UnityEngine.Component>();
