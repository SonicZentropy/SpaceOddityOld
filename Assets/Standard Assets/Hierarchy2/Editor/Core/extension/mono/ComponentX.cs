...
public static class ComponentX {
    public static List<GameObject> xToGOList(IList listComponent) {
        ...
        for (var i = 0; i < listComponent.Count; i++) {
            ...
            result.Add(c == null ? null : (c is UnityEngine.Component) ? (c as UnityEngine.Component).gameObject : null);
        }
        return result;
    }

    public static List<T> xToComponentList<T>(IList goList, bool removeNull = false) where T : UnityEngine.Component {
    ...
    public static void xMove(this UnityEngine.Component c, int delta) {
        while (delta > 0) {
