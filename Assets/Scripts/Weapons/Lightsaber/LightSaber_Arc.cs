...
namespace Zenobit.Weapons.Lightsaber
{
    ...
    [AddComponentMenu("LightSaber/Ray System")]
    public class LightSaber_Arc : MonoBehaviour
    {
        ...
        protected LightSaber_EmitterDestructor[][] emitterDestructors;
        ...
        {
            return new Material(Shader.Find("LightSaber/Additive_core_higlight"));
        }
        ...
        protected void SetFlares(int n)
        {
            ...
            if (arcs[n].flaresOptions.startFlare.enabled)
            {
                ...
                startFlare.brightness = GetFlareBrightness(myCamera.transform.position, resultingShape[0], arcs[n].flaresOptions.startFlare,
            ...
            if (arcs[n].flaresOptions.endFlare.enabled)
            {
                ...
                if (arcs[n].sizeOptions.onlyStartWidth)
                    endFlare.brightness = GetFlareBrightness(myCamera.transform.position, resultingShape[resultingShape.Length - 1], arcs[n].flaresOptions.endFlare,
                                                             arcs[n].sizeOptions.startWidthCurve.Evaluate(elapsedTime / lifetime) / maxStartWidth[n]) * multiplier;
                else
                    endFlare.brightness = GetFlareBrightness(myCamera.transform.position, resultingShape[resultingShape.Length - 1], arcs[n].flaresOptions.endFlare,
            ...
            for (int n = 0; n < arcs.Length; n++)
            {
                ...
                for (int q = 0; q < arcs[n].emissionOptions.Length; q++)
                {
                    if (emitterSystems[n][q] == null && arcs[n].emissionOptions[q].shurikenPrefab != null)
                    {
                        ...
                        GameObject partGameObject = arcs[n].emissionOptions[q].shurikenPrefab.InstantiateFromPool();
                        ...
                        if (emitterSystems[n][q].emission.enabled)
                        {
                            //emitterSystems[n][q].enableEmission = false;

                            var em = emitterSystems[n][q].emission;
                            em.enabled = false;
                        }
                        ...
                        {
                            emitterDestructors[n][q] = partGameObject.AddComponent<LightSaber_EmitterDestructor>();
        ...
        void Start()
        {
            ...
            {
                if ((arcs[i].flaresOptions.startFlare.enabled || arcs[i].flaresOptions.endFlare.enabled) && myCamera == null)
            ...
            for (int n = 0; n < arcs.Length; n++)
            {
                ...
                emitterDestructors[n] = new LightSaber_EmitterDestructor[arcs[n].emissionOptions.Length];
            }
            ...
            //Adding this system to performance engine if it exists
            if (LightSaber_engine.Instance != null)
                LightSaber_engine.Instance.AddArcSystem(this);
        }
            ...
            if (elapsedTime > lifetime)
            {
                switch (playbackType)
                {
                    case ArcsPlaybackType.once:
                            ...
                            for (int i = 0; i < arcs[n].emissionOptions.Length; i++)
                            {
                                try
                                {
                                    if (arcs[n].emissionOptions[i].emitAfterRayDeath)
                                        emitterDestructors[n][i].enabled = true; // no destructor sometimes
                                }
                        ...
                        if (playbackMessages)
                            messageReciever.SendMessage("LightSaberPlayback", this);
                        Destroy(gameObject);
                        break;
