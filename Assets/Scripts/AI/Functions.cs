...
namespace Zenobit.AI
{
    ...
    {
        public static float Logistic(float x, float a = 1.03f, float b = 55f, float c = 8f) => Mathf.Clamp01(a / (1 + b * Mathf.Exp(-c * x)));
        public static float ReverseLogistic(float x, float mul = 2f, float offset = 0.5f) => ToPower(mul * x - 1, 11) + offset;
