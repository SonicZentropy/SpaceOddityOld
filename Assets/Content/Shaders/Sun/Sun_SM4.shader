		...
		SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				if (_Detail >= 1.0) {
					float s1 = noiseSpere(0.5*i.uv1.w, float3(45.78, 113.04, 28.957)*_Seed, fragTime*_SpeedLow);
					s1 = pow(s1*2.4, 2.0);
					float s2 = noiseSpere(4.0*i.uv1.w, float3(83.23, 34.34, 67.453)*_Seed, fragTime*_SpeedHi);
					s2 = s2*2.2;
				...
				fixed c = length(pr)*i.uv1.w;
				pr = normalize(mul(m, pr))*_RayNoiseS.x;//-ray;
				fixed s = max(0.0, (1.0 - abs(_Radius*i.uv1.w - c) / _RayString));//*RayProj;
				...
				pr *= _RayNoiseS.y;
				fixed n = noise4q(float4(pr+ noises01, -fragTime*_SpeedRing + c))*dr;
				pr *= _RayNoiseS.z;
				fixed ns = noise4q(float4(pr+ noises01, -fragTime*_SpeedRay + c))*2.0*dr;
				if (_Detail>=3.0) {
					pr *= _RayNoiseS.w;
					ns = ns*0.5+noise4q(float4(pr+ noises01, -fragTime*_SpeedRay + 0))*dr;
				}
				n = pow(n, _Rays)*pow(nd,_RayRing)*ns;
