		...
		SubShader
	{
		...
		Pass
		{
			...
			fixed noiseSpere(float zoom, float3 subnoise, float anim)
			{
				...
				//if (sphere <sqRadius) {
					if (_Detail>0.0) s = noise4q(fixed4(surfase*zoom*_BodyNoiseS.x + subnoise, fragTime*_SpeedHi+anim))*_BodyNoiseL.x;//*0.625;
					if (_Detail>1.0) s =s*0.85+noise4q(fixed4(surfase*zoom*_BodyNoiseS.y + subnoise*3.0, fragTime*_SpeedHi*3.0+anim))*_BodyNoiseL.y;//*0.125;
					if (_Detail>2.0) s =s*0.94+noise4q(fixed4(surfase*zoom*_BodyNoiseS.z + subnoise*5.0, fragTime*_SpeedHi*5.0+anim))*_BodyNoiseL.z;//*0.0625;//*0.03125;
					if (_Detail>3.0) s =s*0.98+noise4q(fixed4(surfase*zoom*_BodyNoiseS.w + subnoise*6.0, fragTime*_SpeedLow*6.0+anim))*_BodyNoiseL.w;//*0.03125;
					if (_Detail>4.0) s =s*0.98+noise4q(fixed4(surfase*zoom*_BodyNoiseS.w*2.0 + subnoise*9.0, fragTime*_SpeedLow*9.0+anim))*_BodyNoiseL.w*0.36; //0.01125
				//}
