...
public class FX_3DRadar_RID : MonoBehaviour {
	...
	void Start () {
		...
		if(!IsNAV) {
			...
			ThisButton.anchoredPosition3D = Vector3.zero; 
	...
	void UpdateRadar(){
		...
		if(IsNAV && FX3DRM.RenderNAVRadar|| IsPOI){
			if(IsNAV){
				ThisHID.anchoredPosition3D =  GetScreenPos(FX3DRM.HUDIDPadding) + FX3DRM.NAVHUDOffset;
	...
	public void RenderToRadar(bool RenderOnEdge){
		...
		if (newPosA.y < newPosB.y) {
			RIDI.anchoredPosition3D = newPosA + new Vector3(o.x, o.y, 0);
		}else{
			RIDI.anchoredPosition3D = newPosA + new Vector3(o.x, o.y, 0);
		}
		...
		if(!HideVDI || !RenderOnEdge){
			...
			if(FX3DRM.RenderVDI){
				RIDV.anchoredPosition3D = newPosB + NegOffset;
	...
	void RenderHUD(){
		...
		if(!IsNAV && FX3DRM.RenderHUDID && FX3DRM.G_DisplayHUDID && DisplayHUDID){
			...
			ThisHID.anchoredPosition3D = GetScreenPos(FX3DRM.HUDIDPadding) + new Vector3(o.x, o.y, 0);
		}
	}
	...
	void DrawBounds(){
		...
		if(FX3DRM.AdvancedBounds){
			...
			BoundCorner.anchoredPosition3D = new Vector3(newCenter.x, newCenter.y, 0);
			...
			BoundCorner.anchoredPosition3D = new Vector3(Center.x, Center.y, 0);
			BoundCorner.sizeDelta = newSize;
		}
	}
	...
	void MakeRID(){
		...
		if(FX3DRM.RenderRIDB){
			...
			RIDB.anchoredPosition3D = FX3DRM.RIDBOffset;
        ...
        if (FX3DRM.RadarPos == FX_3DRadar_Mgr._radarPos.RenderToTexture) {
            RIDI.localScale = Vector3.one;
            RIDI.localEulerAngles = Vector3.zero;

            if (FX3DRM.RenderVDI) {
                RIDV.localScale = Vector3.one;
                RIDV.localEulerAngles = Vector3.zero;
            }

            if (FX3DRM.RenderRIDB) {
                RIDB.localScale = Vector3.one;
                RIDB.localEulerAngles = Vector3.zero;
            }
        }
	...
	void SetButtonSprite(){
		...
		Vector3 ImagePos = ImageRect.anchoredPosition3D;
		ImagePos.y = Mathf.Round(-ImageHeight * 0.5f);
		ImageRect.anchoredPosition3D = ImagePos;
